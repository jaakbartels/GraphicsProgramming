...
namespace dae {
	...
	{
		return { Vector3::UnitX ,Vector3{0.f, cosf(pitch),sinf(pitch)},Vector3{0.f, -sinf(pitch),cosf(pitch)},Vector3::Zero };
	}

	Matrix Matrix::CreateRotationY(float yaw)
	{
		return { Vector3{cosf(yaw),0.f,-sinf(yaw)},Vector3::UnitY,Vector3{sinf(yaw),0.f,cosf(yaw)},Vector3::Zero};
	}

	Matrix Matrix::CreateRotationZ(float roll)
	{
		return { Vector3{cosf(roll),sinf(roll),0.f} ,Vector3{-sinf(roll),cosf(roll),0.f}, Vector3::UnitZ,Vector3::Zero };
	}
